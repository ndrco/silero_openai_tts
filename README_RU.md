...
- `AUTO_PLAY_SHOW_SKIP_WINDOW` (по умолчанию: `false`) — если `true`, во время серверного воспроизведения показывается локальное окно Tkinter с кнопкой **«Пропустить»**; после завершения кнопка скрывается.
