...
Также в проекте реализовано **авто-проигрывание озвучки** непосредственно на сервере. Этот режим особенно полезен в связи с тем, что в текущей версии  **OpenClaw** встроенное авто-проигрывание со стороны клиента (в браузере **webchat**) работает нестабильно.
...
| Опция | Описание |
|--------|-------------|
| `--host` | Интерфейс для привязки (по умолчанию: `0.0.0.0`) |
| `--port` | Порт для прослушивания (по умолчанию: `8000`) |
| `--force-play` | Принудительно включить проигрывание аудио на сервере (также включает вывод текста) |
| `--show-text` | Выводить текст в консоль перед синтезом |
...
- `SILERO_MODEL_ID` (по умолчанию: `v5_1_ru`) — ID модели Silero для выбранного языка (например: `v5_ru`, `v4_ru`).
...
- `SILERO_DEFAULT_SPEAKER` (по умолчанию: `kseniya`) — спикер, используемый когда `voice` неизвестен/не сопоставлен.
...
- `AUTO_PLAY_SHOW_SKIP_WINDOW` (по умолчанию: `true`) — если `true`, во время серверного воспроизведения показывается локальное окно Tkinter с кнопкой **«Пропустить»**; после завершения кнопка скрывается.
- `FORCE_PLAY` (по умолчанию: `false`) — принудительно включить проигрывание аудио на сервере (переопределение CLI). При включении также выводит текст в консоль перед синтезом.
- `SHOW_TEXT` (по умолчанию: `false`) — выводить текст в консоль перед синтезом (переопределение CLI).
